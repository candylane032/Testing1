...
var check = () => {
    ...
    if (pname != "" && img != "" && !isNaN(p_price) && p_price >= 0) {
                    ...
                    str += '<div class="card text-center pb-2">';
                        str += '<div class="card-body">';
                            ...
                            str += '<p class="' + textColor + '">'+ element.stock +'</p>';
                                str += '<button product_id="' + element.product_id + '" data-bs-toggle="modal" data-bs-target="#updateProduct" class="btn btn-sm btn-secondary mx-1 btn-setId-product">UPDATE</button>';
                                str += '<button product_id="' + element.product_id + '" data-bs-toggle="modal" data-bs-target="#updateProductStock" class="btn btn-sm btn-warning mx-1 btn-setId-productStock">STATUS</button>';
                            str += '</div>';
                        str += '</div>';
                    str += '</div>';
                str += '</div>';
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            json1.forEach(element1 => {
                ...
                str1 += '<button class="btn btn-info btn-sm btn-update-product" product_id="' + element1.product_id + '">UPDATE</button>';
                product1++;
            ...
            $('.btn-update-product').click(function() {
                ...
                if(isNonNegativeNumber(newP_Price)){
                    doRequestUpdateProduct(newPname, newImg, newP_Price, productId);
                }
                else{
...
var doRequestUpdateProduct = (newPname, newImg, newP_Price, productId) => {
    var formData = new FormData();
...
function displaySearch(container, e) {
    ...
    str += '<div class="card text-center pb-2">';
        str += '<div class="card-body">';
            ...
            str += '<div class="buttonssss">';
                str += '<button product_id="' + e.product_id + '" data-bs-toggle="modal" data-bs-target="#updateProduct" class="btn btn-sm btn-secondary mx-1 btn-setId-product">UPDATE</button>';
                str += '<button product_id="' + e.product_id + '" data-bs-toggle="modal" data-bs-target="#updateProductStock" class="btn btn-sm btn-warning  mx-1 btn-setId-productStock">STATUS</button>';
            str += '</div>';
        str += '</div>';
    str += '</div>';
str += '</div>';
